            ...
            for (let i = 0; i < bufData.length - 1; i++) {
                try {
                    if (i < bufData.length - 3) {
                        console.log('testing i: ' + i, bufData[i])
                    ...
                    console.log('digitalMattersFalcon2GDriver error in loop processMessages', e)
                }
            }
        ...
        try {
            ...
            */
            for (let i = 0; i < response.arrBufRawData.length; i++) {
                console.log('splitMultipleMessageData i: ' + i)
                ...
                try {
                    ...
                    for (let x = 5; x < payloadLenTotal; x++) {
                        ...
                        x = x + payloadLenMessage
                    }
                    response = await self.splitMultipleRecordsData(response)
                    deferred.resolve(response)
                } catch (e) {
                    console.log('digitalMattersFalcon2GDriver error in loop splitMultipleMessageData', e)
                }
            }
    ...
    async splitMultipleRecordsData(args) {
        ...
        try {
            for (let i = 0; i < response.arrBufAllData.length; i++) {
                ...
                let bufMessageMessageLen = bufMessage.readUInt16LE(i)
                let sequenceNumber = bufMessage.readUInt16LE(i + 2)
                let rtcDateTime = bufMessage.readUInt16LE(i + 6)
                let logReason = bufMessage.readUInt8(i + 10)
                response.arrBufAllData[i].messageDetails = {
                    sequenceNumber: sequenceNumber,
                    rtcDateTime: rtcDateTime,
                    logReason: logReason
                }
                for (let y = i + 11; y < bufMessage.length; y++) {
        ...
        try {
            ...
            switch (response.msgType) {
                case ('0x00'):    //Hello from Device
                    ...
                    console.log('unitAddress', helloFromDevice.serialNumber)
                    // self.emit('setUnitAddress', { "sock": response.sock, "address": helloFromDevice.serialNumber })
                    ...
                    console.log('Hello response sent to device:', finalBuf)
                ...
                case ('0x05'):
                    ...
                    */
                    ...
                    console.log('Commit response sent to device:', finalBuf)
                    // response.sock.write(finalBuf)
                    response.responseToDevice = finalBuf
                    break
                ...
                case ('0x14'):
                    ...
                    */
                    ...
                    if(bResponseRequired == true){
                        ...
                        console.log('Version response sent to device:', finalBuf)
                        // response.sock.write(finalBuf)
                        response.responseToDevice = finalBuf
                    }
