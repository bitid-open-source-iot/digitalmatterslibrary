...
class protocol{
    ...
    async processTime(digitalMattersTS){
        ...
        try {
            let timeBaseDM = new Date('01/01/2013').getTime()
            let timeDM = Math.floor((new Date(digitalMattersTS + (timeBaseDM/1000)).getTime()))
            // let timeBuf = Buffer.alloc(4)
            // timeBuf.writeUInt32LE(timeDM)
            // deferred.resolve(timeBuf.readUInt32LE())
            deferred.resolve(timeDM * 1000)
        } catch (e) {
